...
from .schemas import AuthUserCreate, AuthUserUpdate, AuthUserResponse, AuthUserPaginateResponse, AuthUserPatch
...
async def auth_users(db: Session = Depends(dbs), user: TokenData = Security(current_user_security, scopes=scopes),
    ...
    data, count, pages = get_paginate_auth_users(db=db, skip=skip, limit=limit, sub_id=user.sub_id)
    return {"code": 0, "message": "", "data": data, "count": count, "pages": pages}
...
async def create_auth_user(auth_user: AuthUserCreate, db: Session = Depends(dbs),
    ...
    auth = verification_sub_id(auth_user, user)
    db_auth_user = get_auth_user_by_name(db=db, name=auth_user.username, sub_id=auth.sub_id)
    if db_auth_user:
